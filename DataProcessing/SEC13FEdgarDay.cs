...
namespace QuantConnect.DataProcessing
{
    ...
    internal static class SEC13FEdgarDay
    {
        ...
        private static readonly Regex IndexLine = new(
            @"^(?<form>13F-HR(?:/A)?)\s+.+?\s+(?<cik>\d+)\s+(?<date>\d{8})\s+(?<file>edgar/data/\S+\.txt)\s*$",
        ...
        /// <summary>One holdings filing listed in a daily index.</summary>
        internal readonly record struct IndexEntry(string FormType, int Cik, DateTime Filed, string Path)
        {
        ...
        /// </summary>
        internal static List<IndexEntry> ParseIndex(string text)
        {
            var entries = new List<IndexEntry>();
            foreach (var line in text.Split('\n'))
            {
                var match = IndexLine.Match(line.TrimEnd('\r'));
                if (!match.Success)
                {
                    continue;
                }
        ...
        /// </summary>
        internal static Filing ParseFiling(IndexEntry entry, string text)
        {
            XElement primary = null;
            var tables = new List<XElement>();

            foreach (Match document in DocumentBlock.Matches(text))
            {
                var body = document.Groups["body"].Value;
                var type = DocumentType.Match(body).Groups["type"].Value.Trim();
                ...
                if (type.StartsWith(PrimaryDocumentTypePrefix, StringComparison.OrdinalIgnoreCase))
                {
                    primary = Parse(xml.Groups["xml"].Value, entry);
                }
                else if (type.Equals(InformationTableDocumentType, StringComparison.OrdinalIgnoreCase))
                {
                    tables.Add(Parse(xml.Groups["xml"].Value, entry));
                }
            }
            ...
            foreach (var line in tables.SelectMany(table => Elements(table, "infoTable")))
            {
        ...
        private static XElement Parse(string xml, IndexEntry entry)
        {
            try
            {
            ...
            {
                throw new InvalidDataException($"SEC13FEdgarDay.Parse(): {entry.Path} carries malformed XML: {err.Message}", err);
            }
        }
