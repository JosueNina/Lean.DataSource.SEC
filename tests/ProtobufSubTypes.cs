...
namespace QuantConnect.DataLibrary.Tests
{
    ...
    public class ProtobufSubTypes
    {
        ...
        public void RegisterSubTypes()
        {
            ...
            RuntimeTypeModel.Default[typeof(BaseData)].AddSubType(2001, typeof(SEC13FHolding));

            // SEC13FHoldings is deliberately absent. A BaseDataCollection implements
            // IEnumerable<BaseData>, and protobuf-net refuses a repeated type as a subclass:
            // "Repeated data (a list, collection, etc) has inbuilt behaviour and cannot be used as a
            // subclass". Its Data list carries no ProtoMember of its own either, so a round trip of
            // the collection would come back empty even if it could be registered. The records
            // inside it serialize on their own, which is what 2001 is for.
        }
    }
}
